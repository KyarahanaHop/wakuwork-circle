...
| `createdAt` | datetime | イベント発生日時  |

**用途**: 入退室履歴の記録（治安運用・分析用）

### 1.10 SupportEvent（応援イベント）

| 属性        | 型       | 説明                                    |
| ----------- | -------- | --------------------------------------- |
| `id`        | UUID     | 主キー                                  |
| `sessionId` | UUID     | FK → Session.id                         |
| `userId`    | UUID     | FK → User.id（応援者）                  |
| `amount`    | int      | 金額（MVP: スタブ値 300/500/1000/3000） |
| `message`   | string?  | 応援メッセージ（最大30文字、改行禁止）  |
| `createdAt` | datetime | 応援日時                                |

**不変条件**:

- `message` は最大30文字
- 表示は時系列順のみ（金額ソート禁止）
- 合計表示・ランキング禁止（D-010参照）
...
Session 1 ──── N PresenceEvent
Session 1 ──── N SupportEvent
...
| PresenceEvent    | `sessionId, createdAt` | 入退室ログ時系列取得     |
| SupportEvent     | `sessionId, createdAt` | 応援イベント時系列取得   |
