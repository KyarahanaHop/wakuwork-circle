...
- **WakuWork Circle制約**: 交換性禁止、治安UI強制、チャット制限、ルーム/セッション分離が遵守されているか（参照: `docs/ssot/core.md`）
...
- [ ] 応援者一覧は時系列のみか（ランキング・累計禁止）

### 治安UI強制チェック
- [ ] ルーム初回参加者はロビー（承認待ち）を経由するか
- [ ] 通報/ブロック導線がワンクリックでアクセス可能か
- [ ] Kick/Mute/Ban/Reportの導線が参加者リストにあるか

### チャット制限チェック
- [ ] 作業中は「スタンプ + カテゴリ + 短文」のみか
- [ ] 休憩時のみテキストチャット可能になっているか
- [ ] ミュート対象は全アクション停止（閲覧のみ）になっているか

### ルーム/セッション分離チェック
- [ ] Room（永続）とSession（一時）が適切に分離されているか
- [ ] セッションコードは自動生成になっているか
- [ ] 合言葉は配信者が手動設定・更新できるか
...
3. 実装者が追加質問なしで進められる
4. WakuWork Circle制約（交換性禁止/治安UI強制/チャット制限/ルーム・セッション分離）が遵守されている
